...
const menuItems: MenuItem[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { id: 'ad-placements', label: 'أماكن الإعلانات', href: '/admin/promotions/ad-placements' },
      // Advanced Ad System
      { id: 'ads-dashboard', label: 'نظام الإعلانات (جديد)', href: '/admin/ads' },
      { id: 'ads-campaigns', label: 'إدارة الحملات', href: '/admin/ads/campaigns' },
      { id: 'ads-packages', label: 'باقات الإعلانات', href: '/admin/ads/packages' },
      { id: 'ads-zones', label: 'مساحات الإعلانات', href: '/admin/ads/zones' },
    ],
  },
  {
    id: 'security',
...
export default function AdminSidebar({
      ...
      <div key={item.id}>
        {hasChildren ? (
          <button
            ...
            className={`sidebar-item flex w-full items-center justify-between px-4 py-3 text-sm font-medium ${depth > 0 ? 'pr-10' : ''} text-slate-300 hover:bg-slate-700/50 hover:text-white`}
          >
            <div className="flex items-center gap-3">
              {item.icon && <item.icon className="h-5 w-5" />}
              <span>{item.label}</span>
              {badge > 0 && (
                <span className="notification-badge min-w-[20px] rounded-full bg-red-500 px-1.5 py-0.5 text-center text-xs font-bold text-white">
                  {badge > 99 ? '99+' : badge}
                </span>
              )}
            </div>
            <ChevronLeftIcon className={`rotate-icon h-4 w-4 ${expanded ? 'expanded' : ''}`} />
          </button>
        ) : (
          <Link
            href={item.href || '#'}
            className={`sidebar-item flex w-full items-center gap-3 px-4 py-3 text-sm font-medium ${depth > 0 ? 'pr-10' : ''} ${
              active
                ? 'active border-l-4 border-blue-500 bg-gradient-to-l from-blue-600/30 to-blue-500/10 text-blue-400'
        ...
        {hasChildren && (
          <div
            className={`sidebar-dropdown ${expanded ? 'expanded' : 'collapsed'} bg-slate-800/50`}
          >
            {item.children!.map((child) => (
              <Link
                key={child.id}
                href={child.href || '#'}
                className={`dropdown-item flex w-full items-center gap-3 px-4 py-2.5 pr-12 text-sm ${
                  isActive(child.href)
  ...
  return (
    <aside className="admin-sidebar fixed right-0 top-0 z-50 flex h-full w-64 flex-col border-l border-slate-700 bg-slate-900">
      {/* Logo */}
      <div className="animate-fade-in border-b border-slate-700 p-5">
      ...
      <div className="border-t border-slate-700/50 bg-gradient-to-t from-slate-950/50 to-transparent p-4">
        ...
        <div className="hover-lift mb-3 rounded-xl bg-slate-800/60 p-3 ring-1 ring-slate-700/50 backdrop-blur-sm">
          <div className="flex items-center gap-3">
            {/* الصورة الرمزية */}
        ...
        {onLogout && (
          <button
            onClick={onLogout}
            className="admin-btn hover-scale-down group flex w-full items-center justify-center gap-2 rounded-xl border border-red-500/20 bg-red-500/5 px-4 py-2.5 text-sm font-medium text-red-400 hover:border-red-500/30 hover:bg-red-500/10 hover:text-red-300 hover:shadow-lg hover:shadow-red-500/10"
          >
