...
export default function EditPackagePage() {
  ...
  const fetchPackage = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/admin/ads/packages/${id}`);
      ...
      setFormData({
        ...
        duration: pkg.duration?.toString() || '7',
  ...
  if (loading) {
    return (
      <AdminLayout title="تعديل الباقة">
  ...
  return (
    <AdminLayout title="تعديل الباقة">
      <Head>
        <title>تعديل الباقة | سوق مزاد</title>
      </Head>
      ...
      <div className="mx-auto max-w-2xl">
        <div className="rounded-2xl border border-slate-700 bg-slate-800 p-6">
          <h1 className="mb-6 text-2xl font-bold text-white">تعديل الباقة</h1>
