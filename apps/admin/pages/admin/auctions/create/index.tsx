/**
 * صفحة إنشاء مزاد جديد - لوحة التحكم
 * Create New Auction - Admin Panel
...
// تمت إزالة YardSelector - مزادات الساحات لها صفحة خاصة في /admin/yards/auctions/create
...
interface FormData {
  ...
  city: string;
  area: string;
  // تمت إزالة yardId - المزادات الأونلاين لا ترتبط بساحات
...
export default function CreateAuctionPage() {
  ...
  useEffect(() => {
    const fetchPromotionPackages = async () => {
      try {
        const res = await fetch('/api/promotion-packages?type=AUCTION');
        if (res.ok) {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    city: '',
    area: '',
    // yardId محذوف - المزادات الأونلاين تظهر في /auctions فقط
  ...
  const handleContinue = async () => {
    if (validateForm()) {
      ...
      localStorage.setItem('adminAuctionData', JSON.stringify(updatedFormData));
  ...
  return (
    <AdminLayout title="إنشاء مزاد جديد">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-white">إنشاء مزاد جديد</h1>
          <p className="mt-2 text-slate-400">أدخل تفاصيل السيارة لإنشاء مزاد جديد</p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-slate-700 bg-slate-800 p-6">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              {/* تمت إزالة YardSelector - لإنشاء مزاد ساحة استخدم /admin/yards/auctions/create */}
          ...
          <div className="rounded-xl border border-amber-500/30 bg-gradient-to-r from-amber-500/5 to-yellow-500/5 p-6">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
              {promotionPackages.map((pkg) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    handleInputChange('promotionPackage', pkg.id);
                    handleInputChange('featured', pkg.price > 0);
                    handleInputChange('promotionDays', pkg.duration);
                  }}
                  ...
                  style={{
                    borderColor: formData.promotionPackage === pkg.id ? pkg.badgeColor : undefined,
                    backgroundColor:
                      formData.promotionPackage === pkg.id ? `${pkg.badgeColor}10` : undefined,
                  }}
                >
                  {formData.promotionPackage === pkg.id && (
                    <div
                  ...
                  <div
                    className="mb-2 w-fit rounded-lg p-2"
                    style={{ backgroundColor: `${pkg.badgeColor}20` }}
                  >
                    <SparklesIcon className="h-5 w-5" style={{ color: pkg.badgeColor }} />
                  </div>
                  <h4 className="text-base font-bold text-white">{pkg.nameAr}</h4>
                  <div className="mt-1 flex items-center justify-between">
                    <span className="text-xs text-slate-400">{pkg.duration} يوم</span>
                    <span className="text-lg font-bold" style={{ color: pkg.badgeColor }}>
                      {pkg.price} د.ل
                    </span>
                  </div>
            ...
            {(() => {
              const selectedPkg = promotionPackages.find((p) => p.id === formData.promotionPackage);
              if (selectedPkg && selectedPkg.price > 0) {
                return (
                  <div
                    className="mt-6 flex items-center justify-between rounded-lg p-4"
                    style={{
                      background: `linear-gradient(to right, ${selectedPkg.badgeColor}10, ${selectedPkg.badgeColor}05)`,
                    }}
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className="rounded-full p-2"
                        style={{ backgroundColor: `${selectedPkg.badgeColor}20` }}
                      >
                        <SparklesIcon
                          className="h-5 w-5"
                          style={{ color: selectedPkg.badgeColor }}
                        />
                      </div>
                      <div>
                        <p className="text-sm text-slate-300">الباقة المختارة</p>
                        <p className="font-bold text-white">{selectedPkg.nameAr}</p>
                      </div>
                    </div>
                    <div className="text-left">
                      <p className="text-sm text-slate-400">المدة</p>
                      <p className="text-lg font-bold" style={{ color: selectedPkg.badgeColor }}>
