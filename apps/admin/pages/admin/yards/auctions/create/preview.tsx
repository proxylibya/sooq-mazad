/**
 * صفحة معاينة ونشر مزاد الساحة - لوحة التحكم
 * Preview and Publish Yard Auction - Admin Panel
 */
...
interface AuctionData {
  ...
  city: string;
  area: string;
  yardId: string; // تم التصحيح من yardName إلى yardId
  contactPhone: string;
  title: string;
  description: string;
...
export default function PreviewYardAuctionPage() {
  ...
  useEffect(() => {
    const savedData = localStorage.getItem('adminYardAuctionData');
    if (!savedData) {
      console.warn('[Preview] لا توجد بيانات محفوظة - إعادة التوجيه');
      router.push('/admin/yards/auctions/create');
      return;
    }
  ...
  const handlePublish = async () => {
    ...
    try {
      const response = await fetch('/api/admin/yards/auctions/create', {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        ...
        localStorage.removeItem('adminYardAuctionData');
        // التوجيه لصفحة الساحات مع رسالة نجاح
        router.push('/admin/yards?auctionCreated=true');
      } else {
    ...
    <AdminLayout title="معاينة المزاد">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-white">معاينة مزاد الساحة</h1>
          <p className="mt-2 text-slate-400">راجع تفاصيل مزاد الساحة قبل النشر</p>
        </div>
        ...
        <div className="mb-6 rounded-xl border border-slate-700 bg-slate-800 p-6">
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
            ...
            {auctionData.yardId && (
              <InfoItem icon={MapPinIcon} label="الساحة" value="مرتبط بساحة ✓" />
            )}
          </div>
        </div>
