    ...
    switch (req.method) {
        case 'GET': {
            try {
                ...
                return res.status(200).json({
                    ...
                    packages: await prisma.promotion_packages.findMany({ where: { isActive: true } }),
                });
            } catch (error) {
        ...
        case 'POST': {
            try {
                ...
                const pkg = await prisma.promotion_packages.findUnique({ where: { id: packageType } });
                const days = customDays || (pkg ? pkg.duration : 7);
