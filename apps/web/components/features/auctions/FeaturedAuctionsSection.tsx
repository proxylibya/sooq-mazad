    ...
    const fetchFeaturedAuctions = async () => {
      try {
        ...
        const transformedAuctions = (result.data || []).map((auction: any) => ({
          ...
          title:
            auction.cars?.title ||
            `${auction.cars?.brand || ''} ${auction.cars?.model || ''}`.trim() ||
            'مزاد سيارة',
          ...
          location: auction.cars?.location || auction.location || 'ليبيا',
          area: auction.area,
          time: auction.endDate ? new Date(auction.endDate).toLocaleDateString('ar-LY') : '',
          images: auction.cars?.images || [],
          condition: auction.cars?.condition || 'جيدة',
          brand: auction.cars?.brand || '',
          model: auction.cars?.model || '',
          year: String(auction.cars?.year || ''),
          mileage: String(auction.cars?.mileage || '0'),
          type: 'auction',
          ...
          image: auction.cars?.images?.[0] || '/images/cars/default-car.svg',
