...
interface GlobalWalletSectionProps {
  balance: number;
  currency: string;
  showBalance: boolean;
...
const GlobalWalletSection: React.FC<GlobalWalletSectionProps> = ({
  balance,
  currency,
  showBalance,
  onToggleBalance,
  ...
  const fetchPaymentMethods = useCallback(async () => {
    try {
      ...
      const cacheKey = 'international_payment_methods';
      ...
      // جلب من API
      const response = await fetch('/api/payment-methods/active?category=international');
  ...
  const formattedMethods = depositMethods.map((method) => ({
    id: method.id,
    name: method.nameAr,
    description: method.description || 'وسيلة دفع دولية',
    icon: method.category === 'bank' ? BanknotesIcon : GlobeAltIcon,
    href: `/wallet/deposit/global/${method.id}`,
    ...
    fees:
      method.percentageFee > 0
        ...
        : method.fixedFee > 0
          ...
          : 'مجاني',
    status: 'active',
    color: method.isPopular ? 'blue' : 'green',
    minAmount: method.minAmount,
    maxAmount: method.maxAmount,
  }));

  // آخر المعاملات العالمية
  ...
  return (
    <div className="space-y-6">
      {/* رأس القسم */}
      <div className="rounded-2xl border border-blue-200 bg-gradient-to-br from-blue-50 via-indigo-50 to-blue-100 p-6">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="rounded-xl bg-blue-200 p-3">
              <GlobeAltIcon className="h-8 w-8 text-blue-700" />
            </div>
            <div>
              <h2 className="text-2xl font-bold text-blue-900">المحفظة العالمية</h2>
              <p className="text-blue-700">إدارة أموالك بالدولار الأمريكي</p>
            </div>
          </div>
          <button
            onClick={onToggleBalance}
            className="rounded-lg bg-blue-200 p-2 text-blue-700 transition-colors hover:bg-blue-300"
          >
        ...
        <div className="text-center">
          <div className="mb-2 text-4xl font-bold text-blue-900">
            {isLoading ? (
              <div className="mx-auto h-12 w-48 animate-pulse rounded bg-blue-200"></div>
            ) : showBalance ? (
          ...
          <p className="text-blue-700">الرصيد المتاح</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6">
        <div className="mb-6 flex items-center gap-3">
          ...
          <h3 className="text-xl font-bold text-gray-900">وسائل الإيداع العالمية</h3>
        ...
        {loadingMethods ? (
          ...
          <div className="rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 p-8 text-center">
            <p className="text-gray-600">لا توجد وسائل دفع دولية متاحة حالياً</p>
                ...
                {method.status === 'active' ? (
                  <Link href={method.href} className="block">
                    <div className="mb-3 flex items-center gap-3">
                      ...
                      <ShieldCheckIcon className="h-5 w-5 text-green-500" title="آمن ومشفر" />
                    </div>
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">${method.maxAmount?.toLocaleString()}</span>
                      </div>
                    </div>
                  </Link>
                ) : (
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6">
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-xl font-bold text-gray-900">آخر المعاملات العالمية</h3>
          </div>
          <Link
            href="/wallet/transactions?type=global"
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">المرجع: {transaction.reference}</p>
                </div>
              </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6">
        <div className="mb-6 flex items-center gap-3">
          ...
          <h3 className="text-xl font-bold text-gray-900">إعدادات المحفظة العالمية</h3>
        </div>

        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <Link
            href="/wallet/settings?section=global"
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">تخصيص إعدادات المحفظة العالمية</p>
              </div>
            </div>
          </Link>

          <Link
            href="/wallet/settings?section=limits"
            className="rounded-lg border border-gray-200 p-4 transition-all hover:border-blue-300 hover:bg-blue-50"
          >
            <div className="flex items-center gap-3">
              <ShieldCheckIcon className="h-5 w-5 text-blue-600" />
              <div>
                <h4 className="font-medium text-gray-900">حدود المعاملات</h4>
                <p className="text-sm text-gray-600">إدارة حدود الإيداع والسحب</p>
              </div>
            </div>
          </Link>
        </div>
...
export default GlobalWalletSection;
