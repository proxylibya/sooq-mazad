  ...
  try {
    ...
    if (carImagesArray && Array.isArray(carImagesArray) && carImagesArray.length > 0) {
      ...
      const validImages = carImagesArray
        .filter((img: any) => img && img.fileUrl && typeof img.fileUrl === 'string' && img.fileUrl.trim())
        .map((img: any) => {
          let url = img.fileUrl.trim();
      ...
      try {
        ...
        if (trimmedImages.startsWith('[') || trimmedImages.startsWith('"')) {
          ...
          if (Array.isArray(parsed)) {
            const validImages = parsed
              ...
              .filter((img: string) => !img.includes('placeholder.com') && !img.includes('via.placeholder'))
              .map((img: string) => {
                const url = img.trim();
                // تصحيح المسارات
                if (url.startsWith('http') || url.startsWith('/')) {
        ...
        if (trimmedImages.includes(',')) {
          ...
          if (splitImages.length > 0) {
            ...
            logger.info(`[ImageResolver] تم استخراج ${splitImages.length} صورة من النص المقسم للمركبة ${vehicleId}`);
            return images;
          }
        }

        // صورة واحدة مباشرة
        if (!trimmedImages.includes('placeholder.com') && !trimmedImages.includes('via.placeholder')) {
          const finalUrl = trimmedImages.startsWith('http') || trimmedImages.startsWith('/')
            ? trimmedImages
            : `/uploads/${trimmedImages}`;
          images.push(finalUrl);
