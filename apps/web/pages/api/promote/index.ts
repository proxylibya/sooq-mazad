...
async function handlePromote(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        const packageConfig = await prisma.promotion_packages.findUnique({
            where: { id: packageType }
        });

        if (!packageConfig || !packageConfig.isActive) {
        ...
        if (paymentMethod === 'wallet') {
            ...
            await prisma.transactions.create({
                data: {
                    ...
                    currency: 'LYD',
                    description: `ترويج ${getEntityTypeName(entityType)} - ${packageConfig.nameAr}`,
                    status: 'COMPLETED',
                    walletId: wallet.id,
        ...
        const endDate = calculatePromotionEndDate(startDate, packageConfig.duration);

        // تحديث الإعلان
        await updateEntityPromotion(entityType, entityId, {
            featured: true,
            promotionPackage: packageType,
            promotionDays: packageConfig.duration,
