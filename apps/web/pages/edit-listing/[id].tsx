...
const EditListingForm = () => {
  ...
  const [listingType, setListingType] = useState<'auction' | 'instant'>('instant');
  ...
  useEffect(() => {
    const loadListingData = async () => {
      ...
      try {
        ...
        let resolvedListingType: 'auction' | 'instant' = 'instant';

        // محاولة جلب بيانات المزاد أولاً (لتحديث إعلانات المزاد باستخدام auctionId أو carId)
        try {
          const auctionResponse = await fetch(`/api/auctions/${id}`);
          if (auctionResponse.ok) {
            const auctionResult = await auctionResponse.json();
            const auctionData = auctionResult.data || auctionResult;
            const vehicle = auctionData.car || auctionData;
            ...
            carData = {
              ...
              price:
                ...
                '',
              contactPhone:
                vehicle.contactPhone ?? auctionData.contactPhone ?? vehicle.contactPhone,
              location: vehicle.location ?? auctionData.location ?? vehicle.location,
              detailedAddress:
                vehicle.detailedAddress ??
                vehicle.locationAddress ??
                auctionData.locationAddress ??
                vehicle.location,
              hasInspectionReport:
                vehicle.hasInspectionReport ?? auctionData.hasInspectionReport ?? false,
              inspectionReportUrl:
                vehicle.inspectionReportUrl ??
                vehicle.inspectionReportFileUrl ??
                auctionData.inspectionReportFileUrl,
              inspectionReport: vehicle.inspectionReport ?? vehicle.manualInspectionData,
              coordinates:
                vehicle.coordinates ??
            ...
            resolvedListingType = 'auction';
          }
        } catch (auctionError) {
          console.error('خطأ في جلب بيانات المزاد:', auctionError);
        }

        // إذا لم يتم العثور على مزاد، نحاول جلب بيانات السيارة مباشرة
        if (!carData) {
          ...
          carData = result.data;
          resolvedListingType = carData?.isAuction ? 'auction' : 'instant';
        }
        ...
        setFormData({
          ...
          price: carData.price?.toString() || carData.startingPrice?.toString() || '',
          ...
          seatCount: carData.seatCount?.toString() || '',
          city: carData.city || carData.location || '',
          area: carData.area || '',
          yardName: carData.yardName || '',
          coordinates: carData.coordinates,
          detailedAddress: carData.detailedAddress || carData.locationAddress || '',
          contactPhone: carData.contactPhone?.replace(/^\+218/, '') || '',
          title: carData.title || '',
          ...
          auctionDuration: '1_week',
          inspectionReport: {
            hasReport: carData.hasInspectionReport || false,
            reportUrl: carData.inspectionReportUrl,
            manualReport: carData.inspectionReport || {
              engineCondition: '',
              bodyCondition: '',
              interiorCondition: '',
              tiresCondition: '',
  ...
  // في وضع التعديل، البيانات محملة مسبقاً من loadListingData
  // لا حاجة لتحميل رقم الهاتف تلقائياً
      ...
      if (!cleanPrice || cleanPrice.trim() === '') {
        ...
        if (!priceValidation.isValid) {
          ...
          if (numericPrice > 50000000) {
            ...
            newErrors.price = 'السعر يبدو منخفضاً جداً. يرجى التحقق من السعر المدخل';
          } else {
  ...
  const handleSave = async () => {
    ...
    try {
      let updatedFormData = { ...formData, listingType: listingType };
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || error.message || 'فشل في حفظ التعديلات');
      }
      ...
      alert(error instanceof Error ? error.message : 'حدث خطأ في حفظ التعديلات');
    }
  };
      ...
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <div className="mb-8">
            ...
            <p className="mt-2 text-gray-600">قم بتعديل بيانات إعلانك وحفظ التغييرات</p>
          </div>
                ...
                <>
                      ...
                      {[
                        {
                          value: 'now',
                          label: 'مزاد مباشر',
                          desc: 'يبدأ المزاد فوراً',
                        },
                        {
                          value: 'after_30_seconds',
                          label: 'بعد 30 ثانية',
                          desc: 'يبدأ المزاد بعد 30 ثانية من النشر',
                        },
                        {
                          value: 'after_1_hour',
                          label: 'بعد ساعة',
                        ...
                        },
                        {
                          value: 'custom',
                          label: 'مخصص',
                          desc: 'حدد وقت بداية المزاد بنفسك',
                        },
                      ].map((option) => (
                        <label
                          key={option.value}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                      {[
                        { value: '1_minute', label: '1 دقيقة', desc: '60 ثانية' },
                        { value: '1_day', label: '1 يوم', desc: '24 ساعة' },
                        {
                          value: '3_days',
                          label: '3 أيام',
                          desc: '72 ساعة',
                        },
                        { value: '1_week', label: 'أسبوع', desc: '7 أيام' },
                        { value: '1_month', label: 'شهر', desc: '30 يوم' },
                      ].map((option) => (
                        <label
                          key={option.value}
                        ...
                        >
                          <input
                            ...
                            onChange={(e) =>
                              handleInputChange(
                                'auctionDuration',
                                e.target.value as FormData['auctionDuration'],
                              )
                            }
          ...
          <div className="mt-8 flex justify-between">
            <button
              onClick={handleBack}
              className="flex items-center gap-3 rounded-lg border-2 border-gray-400 bg-white px-8 py-4 font-medium text-gray-700 transition-all hover:border-gray-500 hover:bg-gray-50 hover:shadow-md"
            >
            ...
            <button
              onClick={handleSave}
              className="flex transform items-center gap-3 rounded-lg bg-green-600 px-10 py-4 text-lg font-medium text-white transition-all hover:scale-105 hover:bg-green-700 hover:shadow-lg"
            >
