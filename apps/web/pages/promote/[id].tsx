/**
 * صفحة ترويج الإعلانات الموحدة - تصميم مضغوط
 * تدعم: السيارات، المزادات، المعارض، خدمات النقل
 */
...
import { EntityType } from '../../lib/promotion/promotion-system';
...
const PromotePage = () => {
  ...
  const { id, type, package: initialPackage } = router.query;

  const listingId = Array.isArray(id) ? id[0] : id || '';

  // تحويل نوع الإعلان - instant و marketplace تعني سيارة في السوق الفوري
  const rawType = Array.isArray(type) ? type[0] : type || 'car';
  const entityType = (['instant', 'marketplace'].includes(rawType) ? 'car' : rawType) as EntityType;
    ...
    const fetchPackages = async () => {
      try {
        const res = await fetch(
          `/api/promotion-packages?type=${entityType === 'car' ? 'LISTING' : entityType.toUpperCase()}`,
        );
        if (res.ok) {
          const data = await res.json();
          // Map API data to UI format if needed, or just use it directly
          const mappedPackages = data.data.map((pkg: any) => ({
            id: pkg.id,
            name: pkg.name,
            nameEn: pkg.nameAr, // Using nameAr as nameEn for now or just name
            price: pkg.price,
            days: pkg.duration,
            priority: 1, // Default
            color: pkg.badgeColor,
            features: pkg.features,
            popular: pkg.price > 20 && pkg.price < 60,
            viewsMultiplier: pkg.price > 50 ? 6 : pkg.price > 20 ? 4 : 2,
          }));
          setPackages(mappedPackages);

          if (mappedPackages.length > 0 && !selectedPackage) {
            // Try to find premium or popular, else first
            const defaultPkg = mappedPackages.find((p: any) => p.popular) || mappedPackages[0];
  ...
  useEffect(() => {
    if (initialPackage && packages.find((p) => p.id === initialPackage)) {
      setSelectedPackage(initialPackage as string);
    }
  ...
  const handlePromote = async () => {
    if (!selectedPackageData || !listingId) return;

    setIsLoading(true);

    try {
      console.log('[Promote Page] إرسال طلب الترويج:', {
        entityType,
        entityId: listingId,
        packageType: selectedPackage,
        paymentMethod: selectedPayment,
        rawType, // النوع الأصلي قبل التحويل
      });
      ...
      console.log('[Promote Page] استجابة API:', { status: response.status, result });
      ...
      // إذا كان الدفع يتطلب بوابة خارجية (ليبيانا/مدار)
      if (result.requiresPayment && result.paymentUrl) {
        toast({
          title: 'جارِ التوجيه للدفع...',
          description: result.message || 'سيتم تحويلك لإكمال الدفع',
        });
        setTimeout(() => router.push(result.paymentUrl), 1000);
        return;
      }

      toast({
        title: 'تم الترويج بنجاح!',
        description: `سيظهر إعلانك كمميز لمدة ${selectedPackageData.days} يوم`,
        variant: 'success',
      });

      // التوجيه للإعلان
      const targetPath =
        entityType === 'auction' ? `/auction/${listingId}` : `/marketplace/${listingId}`;
  ...
  // دالة التخطي والذهاب للإعلان مباشرة
  const handleSkip = () => {
    const targetPath =
      entityType === 'auction' ? `/auction/${listingId}` : `/marketplace/${listingId}`;
  ...
  return (
    <Layout title="ترويج الإعلان" description="اجعل إعلانك مميزاً واحصل على مشاهدات أكثر">
      <Toaster />
      <Head>
        <title>ترويج الإعلان | سوق مزاد</title>
      </Head>
          ...
          <div className="mx-auto max-w-4xl">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 rounded-full bg-white/95 px-3 py-1.5 shadow-md backdrop-blur-sm">
                ...
                <span className="text-xs font-semibold text-orange-600">د.ل</span>
                <button
            ...
            <div className="mt-4 text-center">
              ...
              <h1 className="text-xl font-bold text-white">روّج إعلانك</h1>
              <p className="text-sm text-white/80">مشاهدات أكثر بـ 5 مرات</p>
            </div>
          </div>
        </div>
            ...
            <div className="mx-auto mb-6 max-w-lg">
              <div className="flex flex-col items-center gap-4 rounded-2xl border-2 border-green-300 bg-gradient-to-br from-green-50 to-emerald-50 p-6 shadow-lg">
                ...
                <div className="text-center">
                  <h3 className="text-xl font-bold text-green-800">تم إنشاء الإعلان بنجاح!</h3>
          ...
          <div className="mb-4">
            ...
            <div className="grid grid-cols-4 gap-2">
              ...
              {packages.map((pkg) => {
                ...
                // Dynamic styling based on badgeColor
                const isHex = pkg.color && pkg.color.startsWith('#');
                const style = isHex ? { borderColor: isSelected ? pkg.color : undefined } : {};
                ...
                return (
                  <button
                    ...
                    className={`relative rounded-xl border-2 p-3 text-center transition-all ${
                      isSelected
                        ? `bg-opacity-10 ring-2 ring-opacity-50`
                        : 'border-gray-200 bg-white hover:border-gray-300'
                    }`}
                    style={{
                      ...style,
                      backgroundColor: isSelected && isHex ? `${pkg.color}10` : undefined,
                      boxShadow: isSelected && isHex ? `0 0 0 2px ${pkg.color}30` : undefined,
                    }}
                  >
                    {pkg.popular && (
                      <span
                        className="absolute -top-2 left-1/2 -translate-x-1/2 rounded-full px-2 py-0.5 text-[9px] font-bold text-white"
                        style={{ backgroundColor: pkg.color || '#10b981' }}
                      >
                        الأفضل
                      </span>
                    )}
                    ...
                    <div
                      ...
                      style={{ backgroundColor: pkg.color || '#6b7280' }}
                    >
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex gap-2">
                ...
                <LoadingButton
                  ...
                  className="flex items-center gap-2 rounded-lg bg-gradient-to-r from-amber-500 to-orange-500 px-5 py-2.5 text-sm font-bold text-white shadow-md hover:from-amber-600 hover:to-orange-600 disabled:opacity-50"
                >
                  {isLoading ? (
                    <>
...
// Required for dynamic routes to work properly
export const getServerSideProps: GetServerSideProps = async (
  context: GetServerSidePropsContext,
) => {
  const { id } = context.params || {};
  const { type, package: pkg } = context.query;

  return {
    props: {
      id: id || null,
      initialType: type || 'car',
      initialPackage: pkg || 'premium',
    },
  };
};
