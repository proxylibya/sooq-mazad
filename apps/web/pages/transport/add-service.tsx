...
export default function AddTransportService() {
  ...
  useEffect(() => {
    const fetchPromotionPackages = async () => {
      try {
        const res = await fetch('/api/promotion-packages?type=SERVICE');
        if (res.ok) {
          ...
          if (
            data.data.length > 0 &&
            (!serviceData.promotionPackage || serviceData.promotionPackage === 'free')
          ) {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (serviceData.promotionPackage !== 'free' && data.id) {
          const selectedPkg = promotionPackages.find((p) => p.id === serviceData.promotionPackage);
            ...
            <div className="rounded-3xl bg-gradient-to-b from-slate-50 to-white p-8">
              ...
              <div className="grid grid-cols-2 gap-6 lg:grid-cols-4">
                {promotionPackages.map((pkg) => (
                  <div
                    key={pkg.id}
                    onClick={() => {
                      updateServiceData('promotionPackage', pkg.id);
                      updateServiceData('promotionDays', pkg.duration);
                    }}
                    className={`border-3 relative cursor-pointer rounded-2xl p-8 transition-all duration-300 ${
                      serviceData.promotionPackage === pkg.id
                        ? 'scale-[1.02] border-blue-600 bg-blue-50 shadow-xl'
                        : 'border-slate-200 bg-white hover:border-blue-400 hover:shadow-lg'
                    }`}
                    style={{
                      borderColor:
                        serviceData.promotionPackage === pkg.id ? pkg.badgeColor : undefined,
                      backgroundColor:
                        serviceData.promotionPackage === pkg.id ? `${pkg.badgeColor}10` : undefined,
                    }}
                  >
                    {serviceData.promotionPackage === pkg.id && (
                      <div
                    ...
                    <div className="flex h-full flex-col items-center justify-center text-center">
                      ...
                      >
                        <StarIcon className="h-8 w-8" style={{ color: pkg.badgeColor }} />
                      </div>
              ...
              {(() => {
                const selectedPkg = promotionPackages.find(
                  (p) => p.id === serviceData.promotionPackage,
                );
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        >
                          <StarIcon className="h-7 w-7" style={{ color: selectedPkg.badgeColor }} />
                        </div>
                        <div>
                          <p className="text-xl font-black text-slate-900">{selectedPkg.nameAr}</p>
                          <p className="text-base font-medium text-slate-600">
                            ظهور مميز لمدة {selectedPkg.duration} يوم • سيتم الدفع بعد النشر
                          </p>
                        </div>
                      </div>
