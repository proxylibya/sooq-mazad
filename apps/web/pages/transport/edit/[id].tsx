...
export default function EditTransportService() {
  ...
  const fetchService = async () => {
    try {
      ...
      console.log('[EditService] جلب بيانات الخدمة:', id);
      ...
      console.log('[EditService] استجابة جلب الخدمة:', response.status);

      const data = await response.json();

      if (data.success) {
        console.log('[EditService] تم جلب بيانات الخدمة بنجاح');
        const service = data.service;

        const workingDaysArray = Array.isArray(service.availableDays)
          ? service.availableDays
          : service.availableDays
            ? service.availableDays.split(',').map((day: string) => day.trim())
            : [];
        ...
        setServiceData({
          ...
          serviceRegions: Array.isArray(service.serviceArea)
            ? service.serviceArea
            : service.serviceArea
              ? service.serviceArea.split(',').map((area: string) => area.trim())
              : [],
          ...
          images: Array.isArray(service.images)
            ? service.images
            : service.images
              ? service.images.split(',').map((img: string) => img.trim())
              : [],
        });
      } else {
        console.error('[EditService] فشل في جلب الخدمة:', data.error);

        // إذا كان الخطأ متعلق بالصلاحيات
        if (response.status === 403) {
          setError('ليس لديك صلاحية لعرض هذه الخدمة. تأكد من أنك صاحب الخدمة.');
        } else if (response.status === 404) {
          setError('الخدمة غير موجودة. ربما تم حذفها.');
        } else {
          setError(data.error || 'فشل في جلب بيانات الخدمة');
        }
      }
    } catch (err) {
      console.error('[EditService] خطأ في جلب الخدمة:', err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('[EditService] إرسال طلب التحديث...', { serviceId: id, submitData });
      ...
      console.log('[EditService] استجابة API:', response.status, response.statusText);

      const data = await response.json();
      console.log('[EditService] بيانات الاستجابة:', data);
      ...
      if (data.success) {
        ...
        const errorMsg = data.error || 'فشل في تحديث الخدمة';
        setError(errorMsg);
        console.error('[EditService] فشل في تحديث الخدمة:', errorMsg);

        // إذا كان الخطأ متعلق بالصلاحيات
        if (response.status === 403) {
          setError('ليس لديك صلاحية لتعديل هذه الخدمة. تأكد من أنك صاحب الخدمة.');
        }
      }
    } catch (err) {
      console.error('[EditService] خطأ في تحديث الخدمة:', err);
      setError('خطأ في الاتصال بالخادم. يرجى المحاولة مرة أخرى.');
    } finally {
      setIsLoading(false);
    }
  ...
  const updateServiceData = (key: keyof ServiceData, value: ServiceData[keyof ServiceData]) => {
    setServiceData((prev) => ({
      ...prev,
      [key]: value,
    }));
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                ...
                                updateServiceData(
                                  'serviceRegions',
                                  serviceData.serviceRegions.filter((r) => r !== city),
                                );
                              }
                            }}
