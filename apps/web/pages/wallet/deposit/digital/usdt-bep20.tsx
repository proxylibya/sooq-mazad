/**
 * صفحة إيداع USDT على شبكة Binance Smart Chain (BEP20)
 * USDT BEP20 Network Deposit Page
 */
...
const useBep20Address = (userId?: string) => {
  ...
  useEffect(() => {
    const fetchOrGenerateAddress = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/wallet/address/${userId}?network=bep20`);
        const data = await response.json();

        if (response.ok && data.success && data.data.bep20?.address) {
          setAddress(data.data.bep20.address);
        } else {
          // إنشاء عنوان جديد
        ...
        console.error('Error fetching/generating BEP20 address:', err);
        setError(err.message || 'حدث خطأ في جلب العنوان');

        // Fallback to demo address (Ethereum-style address)
        const hexChars = '0123456789abcdef';
        let generated = '0x';
        for (let i = 0; i < 40; i++)
          generated += hexChars[Math.floor(Math.random() * hexChars.length)];
...
export default function USDTBep20DepositPage() {
  const { user } = useUserContext();
  const { address: bep20Address, loading: addressLoading } = useBep20Address(user?.id);
  ...
  const disabled =
    amount === '' || !!errors.amount || !bep20Address || !user?.id || !confirmedNetwork;

  const copyAddress = async () => {
    if (!bep20Address) return;
    const ok = await copyToClipboard(bep20Address);
    if (ok) {
      setCopied(true);
    ...
    try {
      ...
      const resp = await fetch('/api/wallet/deposit', {
        ...
        body: JSON.stringify({
          ...
          currency: 'USDT-BEP20',
          walletType: 'CRYPTO',
          paymentMethodId: 'USDT_BEP20',
          metadata: { address: bep20Address, network: 'BEP20' },
        }),
      });
  ...
  const quickStats = [
    { label: 'الحد الأدنى', value: '10 USDT', hint: 'شبكة BSC', icon: IconDollarSign },
    { label: 'الاعتمادية', value: '99.0%', hint: 'آخر 30 يوماً', icon: IconShield },
    { label: 'الزمن', value: '1 – 3 د', hint: 'تأكيد الشبكة', icon: IconClock },
  ];

  const timeline = [
    { title: 'نسخ العنوان', desc: 'عنوان BEP20 (صيغة 0x...) على BNB Smart Chain.' },
    { title: 'إرسال المبلغ', desc: 'من محفظتك أو منصتك إلى العنوان.' },
    { title: 'تأكيد الشبكة', desc: 'متابعة لحظية عبر BscScan.' },
    { title: 'إضافة الرصيد', desc: 'تحديث تلقائي للمحفظة.' },
  ];

  const tips = [
    'تأكد أن الشبكة هي BNB Smart Chain (BEP20).',
    'لا ترسل عبر ERC20/Polygon لنفس العنوان.',
    'احفظ رابط BscScan كمرجع للعملية.',
  ];

  return (
    <div dir="rtl" className="min-h-screen bg-slate-950">
      <Head>
        <title>إيداع USDT (BNB Smart Chain) - سوق مزاد</title>
        <meta
          name="description"
          content="إيداع USDT على شبكة BNB Smart Chain (BEP20) بسرعة واعتمادية"
        />
      </Head>
      ...
      <main className="relative mx-auto max-w-7xl px-4 pb-16 pt-10 sm:px-6 lg:px-8">
        <div className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
          <div className="absolute left-1/2 top-20 h-96 w-96 -translate-x-1/2 rounded-full bg-amber-500/20 blur-[140px]" />
          <div className="absolute right-0 top-0 h-72 w-72 translate-x-1/4 bg-yellow-500/20 blur-[120px]" />
        </div>
        ...
        <section className="rounded-3xl border border-white/10 bg-gradient-to-br from-slate-900 via-slate-950 to-black p-6 shadow-[0_25px_70px_rgba(15,23,42,0.55)] sm:p-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <Link
                href="/wallet"
                className="mb-4 inline-flex items-center gap-2 text-sm font-semibold text-amber-200/80 transition hover:text-white"
              >
                <IconArrowLeft className="h-4 w-4" />
                العودة للمحفظة
              </Link>
              <div className="flex flex-wrap items-center gap-3">
                <span className="inline-flex items-center gap-2 rounded-full border border-amber-400/40 bg-amber-400/10 px-4 py-1 text-xs font-semibold text-amber-100">
                  <IconGlobe className="h-4 w-4" />
                  BNB Smart Chain
                </span>
                <span className="inline-flex items-center gap-2 rounded-full border border-yellow-400/30 bg-yellow-400/10 px-4 py-1 text-xs font-semibold text-yellow-100">
                  شبكة BEP20
                </span>
              </div>
              <h1 className="mt-4 text-3xl font-black text-white sm:text-4xl">
                إيداع USDT على BNB Smart Chain
              </h1>
              <p className="mt-3 max-w-2xl text-sm text-slate-300 sm:text-base">
                سرعة جيدة ورسوم منخفضة مع انتشار واسع ودعم عالمي.
              </p>
            </div>
          ...
          <div className="space-y-8">
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6 text-white shadow-xl backdrop-blur-md lg:p-8">
              <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="rounded-full border border-amber-400/50 bg-amber-400/10 px-4 py-2 text-xs font-semibold text-amber-100">
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-5">
                  <div>
                    ...
                    <div className="rounded-2xl border border-white/15 bg-slate-900/40 p-4 font-mono text-sm leading-relaxed text-yellow-50">
                      {addressLoading ? '... جارٍ التحميل' : bep20Address || 'غير متاح'}
                    </div>
                  </div>
                  <div className="flex flex-col gap-3 sm:flex-row">
                    <button
                      ...
                      className="group inline-flex flex-1 items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-amber-500 to-yellow-500 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-amber-500/30 transition hover:scale-[1.01] disabled:pointer-events-none disabled:opacity-50"
                    >
                      {copied ? (
                        <>
                    ...
                    {bep20Address && (
                      <a
                        href={`https://bscscan.com/address/${bep20Address}`}
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/15 px-4 py-3 text-sm font-semibold text-white transition hover:border-amber-300/50"
                      >
                        <IconExternalLink className="h-4 w-4" /> فتح على BscScan
                      </a>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center gap-4">
                  ...
                  <p className="text-xs text-slate-300">امسح الرمز عبر أي محفظة تدعم BSC</p>
                </div>
              </div>
            </div>
              ...
              <div className="mt-8 space-y-8">
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-4 pr-14 text-white placeholder:text-slate-400 focus:border-amber-400"
                    />
                    <span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 rounded-full border border-amber-300/40 bg-amber-300/10 px-3 py-1 text-xs text-amber-100">
                      USDT
                    </span>
                  </div>
                  {errors.amount ? (
                    ...
                    <p className="mt-2 text-xs text-slate-400">الحد الأدنى 10 USDT.</p>
                  )}
                </div>
                ...
                <div className="rounded-2xl border border-amber-400/30 bg-amber-400/10 p-4 text-amber-100">
                  <div className="flex items-start gap-3">
                    ...
                    />
                    <label htmlFor="confirm-bep20" className="text-sm leading-relaxed">
                      <span className="font-semibold">تأكيد مهم:</span> أتعهد بإرسال USDT على BEP20
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-slate-100">
                  ...
                  <div className="mt-3 grid gap-4 sm:grid-cols-2">
                    <div className="rounded-xl border border-white/10 bg-slate-900/40 p-3 text-xs text-slate-300">
                      ...
                      <p className="text-lg font-bold text-white">BNB Smart Chain (BEP20)</p>
                    </div>
                    <div className="rounded-xl border border-white/10 bg-slate-900/40 p-3 text-xs text-slate-300">
                      <p className="text-slate-100">الشفافية</p>
                      <p className="text-lg font-bold text-white">متابعة عبر BscScan</p>
                    </div>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full rounded-2xl bg-gradient-to-r from-amber-500 via-yellow-500 to-orange-500 px-6 py-4 text-lg font-semibold text-white shadow-lg shadow-amber-500/30 transition hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-40"
                >
                  {submitting ? (
                      ...
                      <div>
                        ...
                        {apiRef && (
                          <div className="mt-3 rounded-2xl border border-white/10 bg-white/5 p-3 text-xs text-slate-200">
                            ...
                            <code className="mt-1 block text-base text-amber-200">{apiRef}</code>
                            <Link
                              href="/wallet/transactions"
                              className="mt-2 inline-flex items-center gap-1 text-amber-200 hover:text-white"
                            >
          ...
          <aside className="space-y-8">
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6 text-white shadow-lg backdrop-blur">
              <div className="flex items-center gap-3">
                <span className="rounded-2xl bg-amber-500/10 p-2 text-amber-200">
              ...
              <div className="mt-6 space-y-5">
                {timeline.map((item, idx) => (
                  <div key={item.title} className="flex items-start gap-4">
                    <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full border border-white/20 bg-white/5 text-sm font-bold text-amber-200">
                      {idx + 1}
                    </div>
                    <div>
            ...
            <div className="rounded-3xl border border-rose-400/30 bg-rose-500/10 p-6 text-rose-50">
              ...
              <ul className="space-y-3 text-sm leading-relaxed">
                <li>لا ترسل على ERC20/Polygon – هذا العنوان مخصص لـ BEP20 فقط.</li>
                <li>تحقق من أول وآخر 6 حروف للتأكد من العنوان.</li>
                <li>احتفظ برابط BscScan كمرجع للعملية.</li>
              </ul>
            </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-gradient-to-r from-amber-500/10 to-yellow-500/10 p-6 text-white">
              <h3 className="text-xl font-semibold">هل تحتاج إلى مساعدة فورية؟</h3>
              <p className="mt-2 text-sm text-slate-200">
                فريق الدعم يتابع معاملات BSC على مدار الساعة.
              </p>
              <Link
                href="/support"
