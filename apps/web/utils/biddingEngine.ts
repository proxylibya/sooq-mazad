...
export class BiddingEngine {
   ...
   */
  private async checkUserWallet(_userId: string, bidAmount: number): Promise<BidValidationResult> {
    try {
      ...
      const localWallet = userWallet.localWallet;
   ...
   */
  private async checkUserBan(_auctionId: string, _userId: string): Promise<BidValidationResult> {
    try {
  ...
  > {
    try {
      ...
      if (previousBid && previousBid.bidderId !== newBid.bidderId) {
        return {
   ...
   */
  private hasPendingBid(_auctionId: string, _userId: string): boolean {
    const pendingBids = this.pendingBids.get(auctionId);
    return pendingBids ? pendingBids.has(userId) : false;
  }

  private addPendingBid(_auctionId: string, _userId: string): void {
  ...
  private removePendingBid(_auctionId: string, _userId: string): void {
    const pendingBids = this.pendingBids.get(auctionId);
    if (pendingBids) {
   ...
   * إدارة سجل العروض
   */
  private addToHistory(_auctionId: string, bid: BidData): void {
   ...
   */
  getRecentBids(_auctionId: string, limit: number = 10): BidData[] {
    const history = this.bidHistory.get(auctionId) || [];
    return history.slice(-limit).reverse(); // الأحدث أولاً
  }

  /**
   * مسح سجل العروض للمزاد المنتهي
   */
  clearAuctionHistory(_auctionId: string): void {
    this.bidHistory.delete(auctionId);
    this.pendingBids.delete(auctionId);
    console.log(`🗑️ تم مسح سجل العروض للمزاد ${auctionId}`);
  }

  /**
   * الحصول على إحصائيات المزايدة
   */
  getBiddingStats(_auctionId: string): {
    totalBids: number;
    ...
    const uniqueBidders = new Set(history.map((bid) => bid.bidderId)).size;
